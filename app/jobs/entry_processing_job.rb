class EntryProcessingJob < ApplicationJob
  ...
  def generate_summary(entry)
    chat = RubyLLM.chat(model: "gpt-4.1-mini", provider: :openai, assume_model_exists: true)
  ...
  def generate_embedding(entry)
    text = "#{entry.title}\n#{entry.content}\n#{entry.ai_summary}"
    result = RubyLLM.embed(text.truncate(8000), model: "text-embedding-3-small", provider: :openai, assume_model_exists: true)
    result.vectors
  end
